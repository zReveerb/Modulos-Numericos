...
def chorume(h, s, w, d, Mt, p, mt):                 #h é a taxa anual de chuva na área
    return mt/(h*s*Mt - p*s + mt)                   #S é a área da superfície do aterro sanitário.
...
def aceitavel(C, L):                                #C é a concentraçao de chorume, L é o limite de aceitação dessa concentração
    if  C < L:
        print("QUANTIDADE DE CHORUME ACEITAVEL")
    else:
        print("QUANTIDADE DE CHORUME MAIOR QUE O LIMITE")